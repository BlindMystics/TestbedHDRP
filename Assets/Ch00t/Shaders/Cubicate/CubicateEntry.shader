Shader "Blind Mystics/CubicateEntry"
{
	Properties
	{
		...
		_EntryZoomLength("Entry Zoom Length", Float) = 0.5
		_EntryRegionSize("Entry Region Size", Float) = 0.1
		_EntryCubeSize("Entry Cube Size", Float) = 3

		[PerRenderData]_EntryValue("Entry Value", Float) = 0.0
		[PerRenderData]_DetailValue("Detail Value", Float) = 0.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			#include "CubicateEntryGeometry.hlsl"
			#include "CubicateFragment.hlsl"

			#pragma vertex CubicateVertex
			#pragma geometry CubicateEntryGeometry
